...
**使い方:**
- `/sdd-spec specs/<feature>/intent.md` — パスを直接指定
...
### Step 0: 対象の intent.md を決める

`$ARGUMENTS` が指定されていればそのパスを使う。

**指定がない場合:**  
次のコマンドで `specs/` 以下の `intent.md` を更新日時の新しい順に取得する:
...
>
> 1. `specs/user-auth/intent.md`（10分前）
> 2. `specs/push-notification/intent.md`（昨日）
> 3. `specs/profile-edit/intent.md`（3日前）
>
...
確定したパスの `intent.md` を Read ツールで読み込む。
...
`intent.md` と同じディレクトリに `spec.md` を生成する。  
...
### Step 4: タスク管理ツールへの登録を提案する

spec.md を生成したら、すぐに次を表示する:

> ✅ `specs/[機能名]/spec.md` を生成しました（タスク全 N 件）。
>
> タスクをタスク管理ツールに登録しますか？  
> Mob Elaboration の前に登録しておくと、チームがタスクを確認しながらレビューできます。
>
> 1. Linear
> 2. GitHub Issues
> 3. Obsidian
> 4. スキップ（後で `/sdd-implement` 実行時にも登録できます）
...
3. 各タスクを `save_issue` で登録する:
   - **title:** `[機能名] / [タスク名]`
   - **description:** タスクの内容・成功条件・品質チェックコマンドを含める（末尾に `Spec: specs/[機能名]/spec.md` を付記）
...
### Step 5: Mob Elaboration を促す

タスク登録の完了（またはスキップ）後に次のメッセージを表示する:

> **次のステップ: Mob Elaboration（チームレビュー）を実施してください。**
>
> 確認するポイント:
> - 要件に漏れ・誤りがないか
> - 設計がアーキテクチャと整合しているか
> - タスクが適切な粒度に分割されているか
>
> 承認したら `spec.md` のステータスを `Approved` に変更し、  
> `/sdd-implement specs/[機能名]/spec.md` で実装を開始してください。
